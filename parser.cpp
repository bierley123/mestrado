...
std::string parser::bjt(std::map<std::string, std::string> tokens, int linha, std::string argumento, int qtd_TOKEN)
{
    ...
    std::string generalForm = "General form: QXXXXXXX NC NB NE MNAME";
    std::string parameter_tk;
    std::string msgErro;
    if (qtd_TOKEN == 5)
    {
...
std::string parser::jfet(std::map<std::string, std::string> tokens, int linha, std::string argumento, int qtd_TOKEN)
{
    ...
    std::string generalForm = "General form: JXXXXXXX ND NG NS MNAME";
    std::string parameter_tk;
    std::string msgErro;
    if (qtd_TOKEN == 5)
    {
...
std::string parser::mosfet(std::map<std::string, std::string> tokens, int linha, std::string argumento, int qtd_TOKEN)
{
    ...
    std::string generalForm = "General form: MXXXXXXX ND NG NS NB MNAME ";
    std::string parameter_tk;
    std::string msgErro;
    if (qtd_TOKEN == 6)
    {
...
std::string parser::print(std::map<std::string, std::string> tokens, int linha, std::string argumento, int qtd_TOKEN)
{
    std::string insideNode = "";
    std::string parameter_tk;
    std::string generalForm = "General Form: .PRINT PRTYPE <V(N1<,N2>)> <I(VXXXXXXX)> ";
    std::string msgErro;
    ...
    for (int i = 2; i < qtd_TOKEN;i++)
    {
        if (tokens["token" + std::to_string(i)][0] == 'I')
        {
            if (tokens["token" + std::to_string(i + 1)][0] != 'V')
            {
                msgErro = "Wrong definition '" + (tokens["token" + std::to_string(i + 1)]) + "' , did you mean I(VXXXXXXX) ? ";
                errorPointer(tokens, linha, argumento, generalForm, parameter_tk, msgErro);
            }
        }
        else if (tokens["token" + std::to_string(i)][0] == 'V' && tokens["token" + std::to_string(i-1)][0] != 'I')
        {
            if(tokens["token" + std::to_string(i+2)][0] == ',')
            {
                intValidator(tokens["token" + std::to_string(i+1)],tokens,linha,argumento,generalForm,("token"+std::to_string(i+1)));
                intValidator(tokens["token" + std::to_string(i+3)],tokens,linha,argumento,generalForm,("token"+std::to_string(i+3)));
            }
            else
            {
        ...
        else if(tokens["token" + std::to_string(i)][0] == ',')
        {
            if(tokens["token" + std::to_string(i-2)][0] != 'V')
            {
                //std::cout<<"token:" + tokens["token" + std::to_string(i-2)][0]<<std::endl;
                msgErro = "Wrong definition '" + (tokens["token" + std::to_string(i)]) + "' is misplaced";
                errorPointer(tokens, linha, argumento, generalForm, ("token"+std::to_string(i)), msgErro);
            }
        }
        else if(tokens["token" + std::to_string(i-1)][0] != 'V' && tokens["token" + std::to_string(i-3)][0] != 'V') 
        {
            msgErro = "Wrong definition '" + (tokens["token" + std::to_string(i)]) + "' " + std::to_string(i);
            errorPointer(tokens, linha, argumento, generalForm, ("token"+std::to_string(i)), msgErro);
        }
